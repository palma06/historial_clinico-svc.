package cl.duoc.caso07.historialclinico.cucumber;
...
public class ExpedienteSteps {
    ...
    private Long id;
    private ResponseEntity<Expediente> respuesta;
    private ResponseEntity<Void> respuestaVoid;

    private String url() {
        return "http://localhost:" + port + "/api/expedientes";
    }
    ...
    public void servicioDisponible(String servicio) {
        ResponseEntity<Expediente[]> listado = rest.getForEntity(url(), Expediente[].class);
    ...
    public void crearRecurso(String recurso, String nombre) {
        ResponseEntity<Expediente> creado = rest.postForEntity(
                url(), body(nombre), Expediente.class);
    ...
    public void consultarCreado(String recurso) {
        respuesta = rest.getForEntity(url() + "/" + id, Expediente.class);
    }
    ...
    public void actualizar(String recurso, String nombre) {
        respuesta = rest.exchange(url() + "/" + id, HttpMethod.PUT, body(nombre), Expediente.class);
    }
    ...
    public void consultarEliminado(String recurso, int codigo) {
        ResponseEntity<Expediente> noExiste = rest.getForEntity(url() + "/" + id, Expediente.class);
