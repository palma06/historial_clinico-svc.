...
class ExpedienteControllerTest {
    ...
    private Expediente recurso(Long id) {
        Expediente r = new Expediente();
        r.setId(id);
        r.setNombre("Demo");
        r.setPaciente("valor");
        r.setDiagnostico("valor");
        return r;
    }
    ...
    void obtenerExistenteDevuelve200() throws Exception {
        ...
        mockMvc.perform(get("/api/expedientes/1"))
            .andExpect(status().isOk())
    ...
    void obtenerInexistenteDevuelve404() throws Exception {
        ...
        mockMvc.perform(get("/api/expedientes/1")).andExpect(status().isNotFound());
    }
    ...
    void eliminarExistenteDevuelve204() throws Exception {
        ...
        mockMvc.perform(delete("/api/expedientes/1")).andExpect(status().isNoContent());
    }

    @Test
    void eliminarInexistenteDevuelve404() throws Exception {
        when(service.delete(1L)).thenReturn(false);
        mockMvc.perform(delete("/api/expedientes/1")).andExpect(status().isNotFound());
    }
}
